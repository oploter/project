...
int main() {
  ...
  Player* p = &(model_ptr->getPlayer());//создаем объект p класса player,задаем "hero.png" как имя файла+расширение, далее координата Х,У, ширина, высота.
  ...
  while (window.isOpen()) {
      ...
      while (window.pollEvent(ev)) {
          ...
          if(model_ptr->state == GameState::MENU || model_ptr->state == GameState::DIED){
            if(ev.type == sf::Event::MouseButtonPressed){
                auto mousePos = sf::Mouse::getPosition(window);
                if(model_ptr->getButtons()[0].isClicked(mousePos.x, mousePos.y)){ // нажали "Single game"
                  model_ptr = std::make_unique<Model>("../map.txt");
                  model_ptr->state = GameState::GAME;
                  view.changeModel(*model_ptr);
                  p = &(model_ptr->getPlayer());
                }else if(model_ptr->getButtons()[1].isClicked(mousePos.x, mousePos.y)){ // нажали "Exit"
                  window.close();
                }
              }
              break;
      ...
      if ((sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) && (p->getX() + p->w) < window.getSize().x) {
      ...
      if ((sf::Keyboard::isKeyPressed(sf::Keyboard::Down)) && (p->getY() + p->h) < window.getSize().y) {
      ...
      case GameState::GAME:
          {
          p->update(time, *model_ptr);//оживляем объект p класса Player с помощью времени sfml, передавая время в качестве параметра функции update. благодаря этому персонаж может двигаться
