...
public:
  ...
  void update(float tm, Model &model) {
    switch (dir) {
    case 0:
      dx = speed;
      dy = 0;
...
int main() {
  ...
  while (window.isOpen()) {
    float tm = clock.getElapsedTime().asMicroseconds();
    clock.restart();
    //tm = tm / 500;
    tm /= 20;
    sf::Event ev;
      ...
      if (model_ptr->state == GameState::MENU ||
        ...
        if(ev.type == sf::Event::KeyPressed){
            ...
            if ((code == sf::Keyboard::Left) &&
                ...
                CurrentFrame +=
                    0.015 * tm; //служит для прохождения по "кадрам". переменная доходит
    ...
    case GameState::GAME: {
      p->update(tm, *model_ptr);
      for (std::size_t i = 0; i < bullets.size(); i++) {
        (*bullets[i]).update(tm, *model_ptr);
      }
      model_ptr->update(tm);
      ...
      for(Enemy& enemy : model_ptr->getEnemies()){
        view.drawEnemy(enemy, tm);
      }
      break;
    }
    case GameState::DIED: {
